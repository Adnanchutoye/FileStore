...
class Bot(Client):
    ...
    async def start(self):
        ...
        try:
            ...
            self.LOGGER(__name__).warning(f"Make Sure bot is Admin in DB Channel, and Double check the CHANNEL_ID Value, Current Value {-1001930406310}")
            self.LOGGER(__name__).info("\nBot Stopped. Join https://t.me/AHSS_HELP_ZONE for support")
            sys.exit()

        self.set_parse_mode(ParseMode.HTML)
        self.LOGGER(__name__).info(f"Bot Running..!\n\nCreated by \nhttps://t.me/Anime_Sub_Society")
